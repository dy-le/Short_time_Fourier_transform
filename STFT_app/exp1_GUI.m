...
% --- Executes on button press in show_stft.
function show_stft_Callback(hObject, eventdata, handles)
% hObject    handle to show_stft (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    h = spectrum.welch;
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    axes(handles.axes2);
    title('Spectrogram of Original Signal');
    [S F T P] = spectrogram(handles.x, hamming(512),256,1024, handles.fs);
    plot(T,abs(S));
